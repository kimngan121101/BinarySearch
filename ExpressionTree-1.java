...
public class ExpressionTree extends LinkedBinaryTree<ExpressionTreeOp>
{
	...
	private String inOrderToString(BinaryTreeNode<ExpressionTreeOp> node, String para) {
		String val = "";
		if (node != null) {
			val +=  para+ inOrderToString (node.left, "(") + node.element + inOrderToString (node.right, ")") +para;
		}
		return val ;
	}
    ...
    private String generateCodeNode(BinaryTreeNode<ExpressionTreeOp> root, String v) 
    {
        ...
        else
        {
            ...
            if (element.isOperator())
            {
            	...
            	if (character.contains(element.getVariable()) == false) {
            		character.add(element.getVariable());
            		System.out.print("What is the value of "+ element+"? ");
            		value = scnr.nextInt();
            		scnr.nextLine();
            		integer.add(value);
            		...
            		element.setVariable(integer.get(character.indexOf(element.getVariable())));
            	
            		code = "la " + v + ", " + element.getVariable() + "\n" + "lw " + v + ", "+ value + "(" + v + ")" +"\n" ;
